...
KANBAN_GUIDANCE = (
    ...
    "A hurdle is the next step, never the end: every needs_input/transient block must carry a prepared one-tap next "
    "step (exact link/button + one line of what happens); 'Blocked: needs X' with no prepared action is a defect.\n"
    "**Canon: read before build, write back on finish.** Every card touching a canon-covered area (Sage, Gmail, dash, "
    "feedback) MUST cite what you read in your FIRST comment: `CANON: read canon/<file>#<heading>, ...` (exact format, "
    "see docs/canon/README.md). On completion, pass `metadata={'canon_learned': ['<finding>', ...]}` even if "
    "`['nothing new']` is all you have. Completion is refused and reopened to running if either signal is missing.\n"
