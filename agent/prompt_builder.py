...
KANBAN_GUIDANCE = (
    ...
    "progress.\n"
    "**Durable progress.** Before interruption-prone work, and after verifying an external effect's receipt, save concise "
    "progress with `kanban_checkpoint(action='save', progress={...})`; on a later attempt, worker-context checkpoint "
    "metadata tells you to load it explicitly. A checkpoint is not an arbitrary exactly-once guarantee: inspect the "
    "receipt and current external state before repeating side effects.\n"
    ...
    "detection.\n"
    "6. **If follow-up work appears, create it; don't do it.** For handoff work that should start only "
    "after your task completes, use `kanban_create(title=..., assignee=<right-profile>, "
    "parents=[your-task-id])`. Here `parents` means prerequisites, not ownership. If your programme "
    "must wait for that worker's result, do not put your programme id in the new task's `parents`.\n"
    ...
    "\n"
    "## Orchestrator mode\n\n"
    "If your task is a handoff-only decomposition, use `kanban_create` to fan out specialist tasks with explicit "
    "assignees and prerequisite `parents=[...]`, then `kanban_complete` your own routing task. If your task is a "
    "long-running programme that must inspect child results and report a final outcome, create the first worker "
    "with no prerequisite on your programme, chain later workers to earlier ones, and `kanban_link` the final "
    "reviewer as a prerequisite of your programme (`parent_id=reviewer-id, child_id=your-task-id`) before "
    "dependency-blocking your programme. Never use `parents=[your-task-id]` for a worker whose result your "
    "programme needs before completion; that strands the worker and may cause a duplicate retry. Do not "
    "execute specialist work yourself; retain responsibility for integration and honest final reporting.\n"
    "\n"
