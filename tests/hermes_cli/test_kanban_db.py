...
def test_respawn_guard_canon_gate_refused_bypasses_recent_success(kanban_home):
    """A task whose completion was refused by the acceptance/canon gate and
    sent back to 'ready' via a 'canon_gate_refused' event MUST NOT be held
    by the recent_success guard.

    Before the fix the guard only checked ('status', 'promoted', 'unblocked',
    'reclaimed') as bypass events; 'canon_gate_refused' and 'reconciled' were
    missing, so every canon-refused card sat in ready for up to one hour being
    respawn_guarded every 10 s (t_b3cbc522 postmortem, 2026-09-25).
    """
    import json
    ...
    with kbc.connect() as conn:
        tid = kb.create_task(conn, title="needs-canon-citation", assignee="a")

        # Simulate a completed run ending at `now`.
        kb.claim_task(conn, tid)
        run_id = kb.get_task(conn, tid).current_run_id
        conn.execute(
        ...
        # The acceptance gate fired `canon_gate_refused` and put the task
        # back to ready (orphaned-running reconcile follows).
        conn.execute(
        ...
        conn.execute(
            "INSERT INTO task_events (task_id, kind, payload, created_at) "
            "VALUES (?, 'canon_gate_refused', ?, ?)",
            (tid, json.dumps({"reason": "missing canon_learned"}), now),
        )
        conn.execute(
            "UPDATE tasks SET status='ready', current_run_id=NULL, "
            "claim_lock=NULL, claim_expires=NULL, worker_pid=NULL "
        ...
        # Guard must pass (no recent_success block) because canon_gate_refused
        # is a bypass event meaning the task was not truly done.
        guard = kbd.check_respawn_guard(conn, tid, lane="ready")
        assert guard is None, (
            f"expected no guard after canon_gate_refused, got {guard!r}"
        )


def test_respawn_guard_reconciled_after_completed_bypasses_recent_success(kanban_home):
    """A task whose orphaned-running reconcile fires AFTER a 'completed' run
    (the acceptance gate requeued it) MUST NOT be held by recent_success.

    'reconciled' means the dispatcher re-queued the task after finding it
    running without a live owner - the canonical path when the acceptance gate
    refuses completion and leaves the task as orphaned-running (t_b3cbc522).
    """
    import json
    ...
    with kbc.connect() as conn:
        tid = kb.create_task(conn, title="orphan-reconcile", assignee="a")
        ...
        conn.execute(
            "INSERT INTO task_events (task_id, kind, payload, created_at) "
            "VALUES (?, 'reconciled', ?, ?)",
            (tid, json.dumps({"reason": "orphaned_running"}), now),
        )
        conn.execute(
            "UPDATE tasks SET status='ready', current_run_id=NULL, "
            "claim_lock=NULL, claim_expires=NULL, worker_pid=NULL "
        ...
        guard = kbd.check_respawn_guard(conn, tid, lane="ready")
        assert guard is None, (
            f"expected no guard after reconciled, got {guard!r}"
        )
